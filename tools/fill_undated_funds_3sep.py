...
def main():
    # SUPERSEDED, AND IT REFUSES TO RUN RATHER THAN QUIETLY DOING THE WRONG THING.
    #
    # This script wrote its research straight into data/investors.csv, which is GENERATED by
    # tools/build_investors_table.py. The next rebuild wiped all 27 deals and took thirteen funds
    # off the founder-facing list without a word. The data now lives in
    # data/raw/2026-09-03_curated-fund-deals.csv and the builder reads it as a source, so a
    # rebuild reapplies it instead of destroying it. The dicts below are kept because that raw
    # file was generated from them and they are the record of what was researched.
    print('SUPERSEDED. This script patched a generated file, which is why the deals disappeared.')
    print('The same data is now data/raw/2026-09-03_curated-fund-deals.csv, read by')
    print('tools/build_investors_table.py. Run: python3 tools/build_investors_table.py')
    return 1
