...
| 2026-09-09 | **Growth enrichment of the private rounds** (work order `docs/prompts/growth-enrichment-9sep.md`): for each of the 163 private rounds that price a founder's range and carried no growth figure, either the year-on-year revenue growth at the round with the page read and the sentence quoted, or a reason in plain words why none was found. 97 found (46 DISCLOSED, 25 THIRD_PARTY_DATED, 18 DERIVED, 8 STATED; 20 of them floors such as 'more than doubled'), 66 not found, every row accounted for by name | 163 rows, 135 companies | `data/raw/2026-09-09_private-growth-enrichment.csv` (the filled work list is `docs/growth-enrichment-targets-9sep.csv`) | `data/private-rounds.csv` (92 rows) and `data/private-rounds-consumer.csv` (5 rows) via `tools/load_growth_enrichment_9sep.py`, which writes only `growth_pct_at_round`, `growth_band`, `growth_band_basis`, refuses any row already carrying a figure, and asserts every other byte unchanged; 320 in, 320 out | **yes**, 9-Sep: bands from `selector/match_reference.py` (77 / 206); 20 checks pass; golden 0 of 142 moved because fixtures carry no growth rate; gate 124 of 142 before and after. Write-up `docs/growth-enrichment-9sep.md` |
| 2026-09-10 | **Growth enrichment, round two** (Daniil's rulings of 10 September, plus the rows the 9-September pass had to close on direct page reads because its web-search allowance ran out): for each of the 28 rounds retried, either the year-on-year growth at the round with the page read and the sentence quoted, or a reason in plain words. 11 found (5 DISCLOSED, 3 DERIVED, 2 THIRD_PARTY_DATED, 1 STATED; 4 of them floors), 17 not found, of which 2 are held for a ruling rather than for want of searching | 28 rows, 28 companies | `data/raw/2026-09-10_private-growth-round2.csv` | `data/private-rounds.csv` (8 rows) and `data/private-rounds-consumer.csv` (3 rows) via `tools/load_growth_round2_10sep.py`, same fence as the 9-September loader: three columns only, refuses any row already carrying a figure, asserts every other byte unchanged; 320 in, 320 out | **yes**, 10-Sep: bands from `selector/match_reference.py` (77 / 206, unchanged); 20 checks pass; golden 0 of 142 moved; gate 124 of 142 before and after; the diff was also checked independently of the loader. Write-up `docs/growth-round2-10sep.md` |
